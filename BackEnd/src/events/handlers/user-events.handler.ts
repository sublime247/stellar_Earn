...
export class UserEventsHandler {
  ...
  async handleUserLevelUp(event: IUserLevelUpEvent) {
    this.logger.log(
      `User level up handled: User ${event.userId} to ${event.newLevel}`,
    );
  ...
  async handleReputationChanged(event: IReputationChangedEvent) {
    this.logger.log(
      `User reputation changed: User ${event.userId} changed by ${event.newReputation - event.oldReputation}`,
    );
