...
import {
  ISubmissionReceivedEvent,
  ISubmissionApprovedEvent,
  IPayoutProcessedEvent,
} from '../interfaces/submission-events.interface';
...
export class SubmissionEventsHandler {
  ...
  async handleSubmissionReceived(event: ISubmissionReceivedEvent) {
    this.logger.log(
      `Submission received: ${event.submissionId} for quest ${event.questId}`,
    );
  ...
  async handlePayoutProcessed(event: IPayoutProcessedEvent) {
    this.logger.log(
      `Payout processed: ${event.payoutId} for user ${event.userId}`,
    );
