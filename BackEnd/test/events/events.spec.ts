...
describe('Events Architecture', () => {
  ...
  it('should have listeners registered', () => {
    ...
    expect(eventEmitter.listeners('payout.processed').length).toBeGreaterThan(
      0,
    );
    expect(
      eventEmitter.listeners('submission.rejected').length,
    ).toBeGreaterThan(0);
  });
  ...
  it('should maintain audit log for PayoutProcessedEvent', async () => {
    const spy = jest.spyOn(auditListener['logger'], 'log');
    const event = new PayoutProcessedEvent('payout_1', 'G...', '100', 'hash');
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.stringContaining(
        '[EventAudit] payout.processed: PayoutProcessedEvent',
      ),
    );
  });
  ...
  it('should maintain audit log for PayoutFailedEvent', async () => {
    const spy = jest.spyOn(auditListener['logger'], 'log');
    const event = new PayoutFailedEvent('payout_1', 'G...', 'Error message');
  ...
  it('should maintain audit log for SubmissionRejectedEvent', async () => {
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.stringContaining(
        '[EventAudit] submission.rejected: SubmissionRejectedEvent',
      ),
    );
  });
});
